...
ii_small <- sample_data(BAC_pruned) %>% 
  as_tibble() %>% 
  mutate(row = row_number()) %>% 
  filter(Fraction == 0.22) %>% 
  pull(row)

sample_data(BAC_pruned) <- sample_data(BAC_pruned)[ii_small, ]
...
dat_agg[["OTU"]] <- dat
saveRDS(dat_agg, file = "marine_leucine_small_aggregated.RDS")
