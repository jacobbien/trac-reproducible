...
dat_list <- readRDS("tara_sal_processed_aggregated.RDS")
...
for (j in seq(nsplit)) {
  ...
  for (i in level_names) {
    ...
    fit[[j]][[i]] <- trac(ztr, ytr, A = dat_list[[i]]$A, min_frac = 1e-2,
...
load("tara_sal_trac_fixed_level_multi_splits.Rdata", envir = trac <- new.env())
stopifnot(identical(tr, trac$tr))
save(tr, fit, cvfit, yhat_tr, yhat_te, trainerr, testerr, nnz,
     file = "tara_sal_trac_weights_fixed_level_multi_splits.Rdata")
